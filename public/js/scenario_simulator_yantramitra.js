(function() {
  ...
  function renderKPIs(kpis) {
    ...
    grid.innerHTML = kpiDefs.map(def => {
      ...
      return '<div class="glass-panel rounded-xl p-2 bg-white/70"><div class="flex items-center justify-between mb-1"><div class="flex items-center gap-1"><span class="material-symbols-outlined" style="font-size:13px;color:' + def.color + '">' + def.icon + '</span><span class="text-[8px] font-bold text-on-surface-variant">' + def.label + '</span></div><span class="font-kpi-numeric text-xs" style="color:' + def.color + '">' + val + (def.unit || '') + '</span></div><div style="height:3px;background:#e1dfff;border-radius:2px;overflow:hidden"><div class="kpi-bar" style="height:100%;width:' + pct + '%;background:' + def.color + ';border-radius:2px"></div></div></div>';
    }).join('');
  }
  ...
  function showMachineDetail(machine) {
    ...
    const img = record ? record.image : (currentPlant.image || '/images/home-pune-automotive.jpg');
    panel.innerHTML = '<div class="flex gap-2 mb-2"><img src="' + img + '" class="w-10 h-10 rounded-lg object-cover border border-white/10 shrink-0"/><div class="min-w-0 flex-1"><h4 class="text-white font-bold text-sm truncate">' + escapeHtml(machine.name) + '</h4><p class="text-white/60 text-[9px]">' + escapeHtml(machine.type || 'Machine') + ' · ' + escapeHtml(machine.status || 'operational') + '</p></div><button class="text-white/40 hover:text-white" id="ym-close-detail"><span class="material-symbols-outlined text-sm">close</span></button></div><div class="grid grid-cols-2 gap-1.5 mb-2"><div class="bg-white/10 rounded-lg p-1.5 text-center"><p class="text-white/50 text-[8px] font-bold">HEALTH</p><p class="text-white font-bold text-xs" style="color:' + color + '">' + health + '%</p></div><div class="bg-white/10 rounded-lg p-1.5 text-center"><p class="text-white/50 text-[8px] font-bold">OEE</p><p class="text-white font-bold text-xs">' + (machine.oee || 0) + '%</p></div><div class="bg-white/10 rounded-lg p-1.5 text-center"><p class="text-white/50 text-[8px] font-bold">RUL</p><p class="text-white font-bold text-xs">' + (machine.remainingUsefulLife || 'N/A') + 'd</p></div><div class="bg-white/10 rounded-lg p-1.5 text-center"><p class="text-white/50 text-[8px] font-bold">FAILURE</p><p class="text-white font-bold text-xs">' + (machine.failureProbability || 0) + '%</p></div></div><div class="flex gap-1"><button class="flex-1 bg-primary rounded-lg py-1.5 text-white text-[9px] font-bold hover:opacity-90 transition-opacity" onclick="window.location.href=\'/assets/' + machine.id + '\'">VIEW ASSET</button><button class="flex-1 bg-white/10 rounded-lg py-1.5 text-white/80 text-[9px] font-bold hover:bg-white/20 transition-all" onclick="window.location.href=\'/digital-twin?machine=' + encodeURIComponent(machine.name) + '\'">DIGITAL TWIN</button></div>';
  ...
  function resetSimulation() {
    ...
    renderAll();
    document.getElementById('ym-prediction-text').innerHTML = '<p class="text-white/70 text-[11px]">Adjust parameters and run to generate AI prediction.</p>';
  }
  ...
  function showHistory() {
    ...
    const rows = kpiHistory.slice(-25).reverse().map(entry => {
      return '<tr class="border-b border-outline-variant/10"><td class="py-1 px-1 text-[9px]">#' + entry.iteration + '</td><td class="py-1 px-1 text-[9px]">' + (entry.oee || 0) + '%</td><td class="py-1 px-1 text-[9px]">' + entry.throughput + '</td><td class="py-1 px-1 text-[9px]">' + entry.energy + '</td><td class="py-1 px-1 text-[9px]">' + entry.downtime + '</td><td class="py-1 px-1 text-[9px]">' + formatProfit(entry.profit || 0) + '</td></tr>';
    }).join('');
    openModal('Simulation History (Last 25)', '<table style="width:100%;border-collapse:collapse"><thead><tr style="border-bottom:2px solid #e1dfff"><th class="py-1 px-1 text-[8px] text-left text-on-surface-variant">#</th><th class="py-1 px-1 text-[8px] text-left text-on-surface-variant">OEE</th><th class="py-1 px-1 text-[8px] text-left text-on-surface-variant">Units</th><th class="py-1 px-1 text-[8px] text-left text-on-surface-variant">kWh</th><th class="py-1 px-1 text-[8px] text-left text-on-surface-variant">Down</th><th class="py-1 px-1 text-[8px] text-left text-on-surface-variant">Profit</th></tr></thead><tbody>' + rows + '</tbody></table><p class="text-[9px] text-on-surface-variant mt-2">' + kpiHistory.length + ' total iterations recorded.</p>');
  }
  ...
  function generatePrediction(vals, kpis) {
    ...
    el.innerHTML = '<ul class="text-white/80 text-[10px] space-y-1 ml-3 list-disc">' + insights.slice(0, 3).map(i => '<li>' + i + '</li>').join('') + '</ul><p class="text-white/50 text-[8px] mt-1.5 font-medium">Confidence: ' + confidence + '% · Iteration #' + simIteration + '</p>';
  }
